...
function wc_mmax_custom_add_to_cart($args,$product)
{
    ...
    $minQty     = get_post_meta($product, '_wc_mmax_max', true);
    $maxQty     = get_post_meta($product, '_wc_mmax_min', true);
...
if($cartQty  < $minQty && $mmaxEnable == 1)
{
echo "
<script>
alert('".__('Your Cart quantity is less than the allowed Minimum Quantity '.$minQty.'','wcmax')."');
location.href='". get_permalink($product)."';
</script>";
